...
public class CallScreenActivity extends BaseActivity {
    ...
    private Handler frameThreadHandler;
    ...
    public void onStop() {
        ...
        mTimer.cancel();
        removeVideoViews();
        if(frameThreadHandler != null) {
            frameThreadHandler.removeCallbacksAndMessages(this);
            frameThreadHandler = null;
        }
        if (frameThread != null) {
            frameThread = null;
        }
    ...
    public void onStart() {
        ...
        frameThread = new HandlerThread("frameHandler");
        frameThread.start();
        frameThreadHandler = new Handler(frameThread.getLooper());
        updateUI();
    }
    ...
    private boolean postOnFrameThread(Runnable runnable) {
        synchronized (handlerLock) {
            return frameThreadHandler != null
                    && frameThreadHandler.post(runnable);
        }
    }
    ...
    private void addLocalView() {
        ...
        if (vc != null) {
            ...
            //Add a listener to process frames before it's sent to the remote client (convert them to Black-and-White)
            vc.setLocalVideoFrameListener((yuvFrame, videoFrameCallback) -> postOnFrameThread(() ->
            {
                VideoUtils.convertToBlackAndWhite(yuvFrame);
                videoFrameCallback.onFrameProcessed();
            }));
        }
    }
